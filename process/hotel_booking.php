...
try {
    ...
    try {
        ...
        if (file_exists($autoloadPath)) {
            ...
            // Configure SMTP if available; default is mail()
            // $mail->isSMTP();
            ...
            $adminTo = 'admin@localhost'; // TODO: set admin email
            $mail->setFrom('no-reply@localhost', 'Hotel Booking');
            $mail->addAddress($adminTo);
            if ($email) {
                $mail->addReplyTo($email);
            }
            $mail->isHTML(true);
            $mail->Subject = 'New Hotel Booking #' . $booking_id;
