  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Hotel Booking Manager</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <section class="section dashboard">
      <?php
        ...
        // Ensure status column exists (safe check)
        $hasStatus = false;
        $chk = $conn->query("SHOW COLUMNS FROM hotel_bookings LIKE 'status'");
        if ($chk && $chk->num_rows > 0) { $hasStatus = true; }
        if ($chk) { $chk->close(); }
        if (!$hasStatus) {
          // Add column if missing
          @$conn->query("ALTER TABLE hotel_bookings ADD COLUMN status VARCHAR(20) NOT NULL DEFAULT 'pending'");
        }

        // Fetch hotel bookings
        $rows = [];
        $sql = "SELECT id, destination, checkin, checkout, adults, children, mobile, email, created_at,
      ...
      <div class="card">
        <div class="card-body">
          <h5 class="card-title">All Hotel Bookings</h5>
                ...
                <?php else: ?>
                  <?php foreach ($rows as $bk): ?>
                    <tr data-id="<?= (int)$bk['id'] ?>">
                      ...
                      <td><?= htmlspecialchars($bk['checkin']) ?></td>
                      <td><?= htmlspecialchars($bk['checkout']) ?></td>
                      <td><?= (int)$bk['adults'] ?></td>
        ...
        (function(){
          ...
          document.addEventListener('change', function(e){
            if(e.target && e.target.classList.contains('bk-status')){
              ...
              fetch('process/hotel_booking_status.php', { method:'POST', body: form })
